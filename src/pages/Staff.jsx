...
import { Phone, MapPin, UserCheck, Send } from 'lucide-react';
import Topbar from '../components/Topbar';
import { STAFF } from '../data/mockData';
import { logDispatch } from '../lib/supabaseService';
...
export default function Staff({ sidebarOpen, setSidebarOpen }) {
  ...
  const dispatch = async () => {
    ...
    const entry = { staffId: selected.id, msg: taskMsg, time: new Date().toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' }) };
    setSending(true);
    try {
      await logDispatch({ staffId: selected.id, message: taskMsg });
    } catch (e) {
      console.warn('Dispatch log to DB failed (using local):', e.message);
    } finally {
      setSending(false);
    }
