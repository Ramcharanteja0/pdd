...
export async function resolveIncident(id) {
  ...
  // If resolved_at column doesn't exist yet (migration not run), fall back
...
export async function resolveAlert(id) {
  // Try updating with resolved_at timestamp first
  const { data, error } = await supabase
    .from('alerts')
...
export async function buildCrowdTimeline() {
  ...
  // Build hourly bins from 08:00 to 22:00 (10 PM) for a fixed full-day view
  const startHour = 8;
  const endHour = 22;
  const hourMap = {};
